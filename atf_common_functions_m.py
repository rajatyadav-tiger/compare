...
def set_azure_connection_config(connectionconfig,spark):
  ...
  sas_token = connectionconfig['SAS_TOKEN']  # SAS token for ADLS access

  spark.conf.set(f"fs.azure.account.auth.type.{storage_account}.dfs.core.windows.net", "SAS")
  spark.conf.set(f"fs.azure.sas.token.provider.type.{storage_account}.dfs.core.windows.net", "org.apache.hadoop.fs.azurebfs.sas.FixedSASTokenProvider")
  spark.conf.set(f"fs.azure.sas.fixed.token.{storage_account}.dfs.core.windows.net", sas_token)
  return container_name,storage_account
...
def readconnectionconfig(connectionname):
  ...
  connection_config=json.load(open("/Workspace/Shared/DEV/edp_qa_auto_tester/datf_core/test/connections/"+connectionname+".json"))
