...
class S2TAutoLoadScripts:
  ...
  def getSelectTableCmd(self, loadLayer):
    ...
    autoScriptFile = autoScriptFile.replace("file:", "")

    # dataFile = f"file:{dataFile}"

    print("PATH of file is ", dataFile)
    print("Format of file is ", dataFormat)
    ...
    if dataFormat in ["avro","delta","parquet","json","delimitedfile"]:
      ...
      print('Go to read_data function manual')
      f.write(f'spark.sql("{self.selectTableCommand}")\r\n')
