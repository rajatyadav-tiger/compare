...
class S2TTester:
    ...
    def starttestexecute(self, protocol_file_path, testcasetype, testcasesrunlist):
        ...
        try:
            ...
            #Added by Sid line 81
            root_path_workspace= root_path.replace('file:','')
            results_path = str(root_path_workspace+dict_protocol['protocol_results_path'])
            ...
            combined_testcase_output_path = protocol_output_path + "run_"+testcasetype+"_tc_combined_" + \
                str(dict_protocol['protocol_name']) + \
                "_" + created_time + ".pdf"
    ...
    def execute_testcase(self, tc_config, auto_script_path, testcasetype):
        ...
        if tc_config['comparetype'] == 's2tcompare':
            loads2t_path = root_path.replace("file:","")+tc_config['s2tpath'] # added replcae Sid
    ...
    def compare_data(self, compare_input, testcasetype):
        ...
        # print("Printing source dataframe:")
        # sourcedf.display()
        # print("Printing Target dataframe:")
        # targetdf.display()
    ...
    def generate_protocol_summary_report(self, df_protocol_summary, protocol_run_details, protocol_run_params, output_path, created_time, testcasetype):
        ...
        print("Generate pdf func called")
        ...
        for test_result in test_results_list:
            ...
            print("Section heading executed")
            if (df_protocol_summary is not None):
                print("Inside if.")
                print("Filtering df_protocol_summary")
                df_protocol_summary_temp = df_protocol_summary.select(
                    '*').filter(col('Test Result') == test_result)
                print("Filter performed, proceeding to print schema and count")
                df_protocol_summary_temp.printSchema()
                print("After print schema")
                print(df_protocol_summary_temp.count())
                print("After head and count")
                if (df_protocol_summary_temp.count() == 0):
                    print("count cond satisfied")
                    df_protocol_summary_temp = None
                print("After count")
            else:
                print("Inside else")
                df_protocol_summary_temp = None
            print("Before calling create table details")
        ...
        protocol_output_path = output_path + "run_"+testcasetype+"_" + \
            ...
            "_" + created_time+".pdf"
        print(f"Protocol path : {protocol_output_path}")
    ...
    def concat_keys(self, df, key_cols_list):
            ...
            print(keycols_name_temp[0],type(keycols_name_temp[0]))
            print(keycols_val_temp[0],type(keycols_val_temp[0] ))
            df.show()

            for i in range(len(keycols_name_temp)):
                # print('Key -{}, value -{}').format(keycols_name_temp[i],keycols_val_temp[i])
                # print('Keytype -{}, valuetype -{}').format(type(keycols_name_temp[i]),type(keycols_val_temp[i]))
                print(f"Key -{keycols_name_temp[i]}, value -{keycols_val_temp[i]}")
                print(f"Keytype -{type(keycols_name_temp[i])}, value -{type(keycols_val_temp[i])}")
